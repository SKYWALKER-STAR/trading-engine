...
正文保留 `1a7b7ff` 复核时点。随后实现了 [执行仓位成本与百分比平仓](execution-cost-and-price-exits.md)：补充持仓均价及成本完整性，修复 F02 的部分成交撤单/部分平仓处理，以及 F03 的最终 positionSide/reduceOnly 传递。新退出规则默认关闭；下文相关“待修复”描述属于原复核基线，当前实现与验证边界以功能说明为准。
